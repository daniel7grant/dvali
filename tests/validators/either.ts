...
const testFailingValidation = (i: number): ValidatorFunction =>
    async function (_value, _conf) {
        return Failure(`test ${i} failed`);
    };

const testTransformingValidation = (): ValidatorFunction =>
...
test('either function resolves to first if the first is transformation', async () => {
    const value = 1;
    const validateTest = validate(either([testTransformingValidation(), testSuccessfulValidation()]));
